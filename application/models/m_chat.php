<?php
	class m_chat  extends CI_Model  {
		...
		function profile($nim){
		    $sql=$this->db->query("SELECT `nama`, `posisi` FROM `pegawai` WHERE status='dokter'");
		    return $sql;
		}

		function kirim_chat($nim,$cht,$pengirim,$waktu,$pegawai_id){
		    $sql=$this->db->query("INSERT INTO `chat`(`pasien_nip_nim`, `cht`, `pengirim`, `waktu`, `pegawai_id`) VALUES ('$nim','$cht','$pengirim','$waktu','$pegawai_id')");
		    return $sql;
		}
